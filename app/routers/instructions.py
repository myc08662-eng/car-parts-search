...
INSTRUCTIONS_DIR = os.path.join(BASE_DIR, 'static', 'manuals')
...
async def download_instruction(instruction_id: int):
    ...
    if not full_path.startswith(os.path.abspath(INSTRUCTIONS_DIR)):
        logger.warning(f"Попытка доступа за пределы разрешённой директории: {full_path}")
